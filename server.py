...
def inv_handler(csock, caddr):
    ...
    while not breaking_out:
        ...
        if header in headers:
            csock.send("Duplicate order recieved".encode('utf-8'))
            breaking_out = True
            break
        ...
        for order in lines.items():
            ...
            with threading.Lock():
                ...
                # register sequence order was rcvd in, and
...
if __name__== '__main__':
    ...
    global inv
    inv = OrderedDict()
    # dict to keep orders as they are recieved to output when inventory reaches zero
    global ordersmap
